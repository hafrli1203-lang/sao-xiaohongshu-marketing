...
export default function ConsumerTrends() {
  const getIcon = (iconStr: string) => {
    switch(iconStr) {
      case 'won': return <ShoppingBag className="w-7 h-7 sm:w-10 sm:h-10 text-xhs-red" />;
      case 'travel': return <Plane className="w-7 h-7 sm:w-10 sm:h-10 text-amber-500" />;
      case 'korea': return <Heart className="w-7 h-7 sm:w-10 sm:h-10 text-pink-500" />;
      default: return <ShoppingBag className="w-7 h-7 sm:w-10 sm:h-10 text-xhs-red" />;
    }
  };

  return (
    <section id="consumer-trends" className="py-16 sm:py-32 bg-white relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 z-10">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {consumerTrendsData.map((card, idx) => (
            <div 
              key={card.title} 
              className="bg-white rounded-3xl p-6 sm:p-8 border border-gray-100 shadow-[0_10px_30px_rgba(0,0,0,0.02)] hover:shadow-[0_20px_50px_rgba(0,0,0,0.08)] transition-all duration-300 group hover:-translate-y-1 relative overflow-hidden"
            >
              {/* Subtle top border highlight */}
              <div className={`absolute top-0 left-0 w-full h-1 ${idx === 0 ? 'bg-xhs-red' : idx === 1 ? 'bg-amber-500' : 'bg-pink-500'}`}></div>
              
              <div className="mb-6 sm:mb-8 p-3 sm:p-4 bg-gray-50 rounded-2xl w-fit group-hover:scale-110 transition-transform duration-500">
                {getIcon(card.icon)}
              </div>
              <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-6">{card.title}</h3>
